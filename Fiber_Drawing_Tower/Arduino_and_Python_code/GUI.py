...
def serial_ports():
    ...
    if result == []:
        # If no port are available, the "None" string is returned (this is a quick fix that made it work). A regular None type should be better and replaced in the future
        return ["None"]
    return result
...
class FiberTower():
    ...
    def load_config(self):
        ...
        try:
            ...
            self.config_data = {
                ...
                "microstepping": 1600,
                "preform_length": 50
                # Microstepping is for the capstan and spool (both need to be the same)
            }
    ...
    def send_config(self):
        ...
        """
        dp2vp = str(self.config_data["preform_diameter"]**2 * self.config_data["preform_linear_speed"]
                    # Preform diameter^2 * Preform linear speed. Send one number instead of two.
                    )
        self.port_write(dp2vp)
        self.port_write('f')

        # Capstan wheel diameter
        dc = str(self.config_data["capstan_wheel_diameter"])
        self.port_write(dc)
        self.port_write('g')

        # Spool circumference
        ds = str(self.config_data["spool_circumeference"])
        self.port_write(ds)
        self.port_write('h')

        # Microstepping of capstan and spool
        micro = str(self.config_data["microstepping"])
    ...
    def createGui(self):
        ...
        # Tells tkinter to execute the close_window function when the app is closed
        self.root.protocol("WM_DELETE_WINDOW", self.close_window)

        # PREFORM STEPPER SECTION

        # Frame
        self.preform_frame = tk.LabelFrame(
            self.root, text="Preform Motor", height=100, width=150)
        self.preform_frame.grid(
            row=0, column=0, rowspan=3, columnspan=3, padx=5, pady=5)

        # Buttons of preform section

        self.label_up = tk.Label(
            self.preform_frame, text="UP", font=("Segoe Ui", 12))
        self.label_up.grid(row=0, column=0, padx=5)
        self.button_up = ttk.Button(
            self.preform_frame, text=u"\u2191", command=lambda: self.port_write("t"))
        self.button_up.grid(row=0, column=1, padx=5)

        self.button_stop = tk.Button(self.preform_frame, text="Stop", command=lambda: self.port_write(
            "k"), font=(12), bg="red", fg="white")
        self.button_stop.grid(row=1, column=1, padx=5)

        self.label_down = tk.Label(
            self.preform_frame, text="DOWN", font=("Segoe Ui", 12))
        self.label_down.grid(row=2, column=0, padx=5)
        self.button_down = ttk.Button(
            self.preform_frame, text=u"\u2193", command=lambda: self.port_write("b"))
        ...
        self.capstan_frame = tk.LabelFrame(
            self.root, text="Capstan Motor", height=120, width=150)
        self.capstan_frame.grid(row=3, column=0, rowspan=2, columnspan=3)

        # Creation of green Start button
        self.button_start = tk.Button(self.capstan_frame, text="Start", command=lambda: self.port_write(
            "s"), font=("Segoe Ui", 12), bg="green", fg="white")
        self.button_start.grid(row=0, column=0, padx=5)

        # Creation of red Stop button
        self.button_stop = tk.Button(self.capstan_frame, text="Stop", command=lambda: self.port_write(
            "a"), font=("Segoe Ui", 12), bg="red", fg="white")
        ...
        self.spool_frame = tk.LabelFrame(
            self.root, text="Spool Motor", height=120, width=150)
        self.spool_frame.grid(row=5, column=1, rowspan=2, columnspan=3, padx=5)

        # Creation of green Start button
        self.spool_start = tk.Button(self.spool_frame, text="Start", command=lambda: self.port_write(
            "w"), font=("Segoe Ui", 12), bg="green", fg="white")
        self.spool_start.grid(row=0, column=0, padx=5)

        # Creation of red Stop button
        self.spool_stop = tk.Button(self.spool_frame, text="Stop", command=lambda: self.port_write(
            "q"), font=("Segoe Ui", 12), bg="red", fg="white")
        self.spool_stop.grid(row=0, column=1, padx=5)

        # Creation of yellow Reverse button
        self.spool_stop = tk.Button(self.spool_frame, text="Reverse", command=lambda: self.port_write(
            "r"), font=("Segoe Ui", 12), bg="yellow", fg="black")
        ...
        self.parameter_frame = tk.LabelFrame(
            self.root, text="Parameters", height=100, width=150)
        self.parameter_frame.grid(
            row=2, column=4, rowspan=4, columnspan=3, padx=5)
        ...
        self.diameter_desired = tk.Label(
            self.parameter_frame, text='Diameter desired :')
        ...
        self.diameter_entry_button = tk.Button(
            self.parameter_frame, text='Send', command=self.send_diameter)
        self.diameter_entry_button.grid(row=1, column=2)

        # Record diameter button
        self.record_button = tk.Button(
            self.parameter_frame, text="Record diameter", command=self.record_diameter)
        ...
        # length drawn
        self.length_drawn = tk.Label(
            self.parameter_frame, text="Length drawn: ")
        ...
        self.connection_frame = tk.LabelFrame(
            self.root, text="Connection", height=50, width=150)
        self.connection_frame.grid(
            row=0, column=4, rowspan=2, columnspan=2, padx=5, pady=5)

        # Port selection menu

        self.connect_button = tk.Button(
            self.connection_frame, text="Reconnect", command=self.reconnect)
        self.connect_button.grid(row=0, column=1, padx=5)

        self.connect_button = tk.Button(
            self.connection_frame, text="Check Ports", command=self.check_ports)
        self.connect_button.grid(row=1, column=1, padx=5)

        self.status_label = tk.Label(
            self.connection_frame, text="Status: Disconnected", bg='red')
        ...
        self.stop_all_button = tk.Button(self.stop_all_frame, text="Stop", bg="red", font=(
            "Segoe Ui", 12), fg="white", command=lambda: self.port_write("akq"))
        self.stop_all_button.grid(column=1, row=0)

        self.start_all_button = tk.Button(self.stop_all_frame, text="Start", bg="green", font=(
            "Segoe Ui", 12), fg="white", command=lambda: self.port_write("sbw"))
        ...
        self.connection_drop_menu = tk.OptionMenu(
            self.connection_frame, self.current_port, *serial_ports())
        self.connection_drop_menu.grid(row=0, column=0, columnspan=1, padx=5)

        # Live diameter visualisation

        self.visualiser_frame = tk.LabelFrame(
            self.root, text="Live diameter graph", height=300, width=300)
    ...
    def record_diameter(self):
        ...
        if self.recording:
            ...
            # The pandas library is used to save the diameter data
            df = DataFrame(self.save_data)
            df.to_csv(
                f'../Drawing_data/{datetime.today().strftime("%Y%m%d, %Hh%Mm%Ss")}.csv', index=False)
        else:
            self.recording = True
            self.buffer = []
            self.save_data = {
            ...
            self.drawn_volume = 0
            self.preform_volume = self.config_data["preform_diameter"]**2 * \
                np.pi * self.config_data["preform_length"] / 4

    def check_ports(self):
        """Updates the connection drop menu with available ports"""

        # I found no way to modify the menu options so the menu is deleted and a new one is created
        ...
        for port in ports:
            self.connection_drop_menu['menu'].add_command(
                label=port, command=tk._setit(self.current_port, port))
    ...
    def initialization(self, commPort):
        ...
        if commPort == "None":
            ...
            # Waits that the serial connection is well established before sending the config
            sleep(2)
            self.send_config()
            ...
            if self.ser.isOpen() and self.ser.in_waiting:
                ...
                if debug:
                    ...
                    self.time_debug.config(text=f"Time difference: {
                        round((time()-self.previous_time) * 1000)} ms")
                    self.previous_time = time()
